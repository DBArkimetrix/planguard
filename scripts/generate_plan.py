...
def infer_allowed_paths(prompt: str) -> list[str]:
    ...
    if any(x in p for x in ["api", "endpoint", "route", "controller"]):
        paths.extend(["src/api", "tests/api"])
    if any(x in p for x in ["schema", "database", "migration", "sql", "table"]):
        paths.extend(["src/db", "migrations", "tests/db"])
    if any(x in p for x in ["ui", "frontend", "screen", "page"]):
        paths.extend(["src/ui", "tests/ui"])
    if any(x in p for x in ["service", "engine", "pricing", "logic"]):
...
def main():
    ...
    dependency_map = {
        "dependencies": [
            {
                "component": "unknown_initially",
                "impact": "to be refined during analysis phase",
            }
        ]
    }
    ...
    collision_detection = {
        "collision_checks": [
            {"check": "overlapping file modifications"},
            {"check": "schema conflicts"},
            {"check": "endpoint contract conflicts"},
            {"check": "service boundary violations"},
            {"check": "unintended downstream consequences"},
        ]
    }
